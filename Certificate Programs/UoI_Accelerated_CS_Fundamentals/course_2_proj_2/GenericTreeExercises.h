...
static void treeFactory(GenericTree<int>& tree) {
	...
	// check if there are already contents
	// if so, delete
	if (tree.getRootPtr()) tree.clear();

	tree.createRoot(4);
	auto root = tree.getRootPtr();

	auto rootLeftChild = root->addChild(8);
	auto rootRightChild = root->addChild(15);

	auto rLCLeftChild = rootLeftChild->addChild(16);
	rootLeftChild->addChild(23);

	rLCLeftChild->addChild(42);	
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  frontier.push(rootNodePtr);

  while(!frontier.empty())
  {
	TreeNode* nodePtr = frontier.front();
	frontier.pop();

	if (nodePtr)	
	{
		results.push_back(nodePtr->data);
		for (TreeNode* node : nodePtr->childrenPtrs)
			frontier.push(node);
	}
  }

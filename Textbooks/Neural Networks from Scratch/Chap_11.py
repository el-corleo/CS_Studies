...
class Optimizer_SGD:
	...
	def __init__(self, starting_learning_rate=1.0, decay_rate=0.0001, momentum=0.0):
		self.curr_learning_rate = starting_learning_rate
	...
	def update_params(self, layer):
		if self.momentum:
			...
			updated_weights = self.momentum * layer.weights_momenta - self.curr_learning_rate * layer.dweights
			updated_biases = self.momentum * layer.biases_momenta - self.curr_learning_rate * layer.dbiases
		else:
			updated_weights = -self.curr_learning_rate * layer.dweights
			updated_biases = -self.curr_learning_rate * layer.dbiases
	...
	def decay_learning_rate(self):
		...
		if self.curr_learning_rate - self.decay_rate > 0:
			self.curr_learning_rate -= self.decay_rate
...
class Optimizer_AdaGrad():
	...
	def update_params(self, layer):
		...
		layer.weights += -self.curr_learning_rate * layer.dweights / (np.sqrt(layer.weights_cache) + self.epsilon)
		layer.biases += -self.curr_learning_rate * layer.dbiases / (np.sqrt(layer.biases_cache) + self.epsilon)
...
class Optimizer_Adam:
    ...
    def update_params(self, layer):
        ...
        layer.weights += -self.curr_learning_rate * weights_momenta_corrected / (np.sqrt(weights_cache_corrected) + self.epsilon)
        layer.biases += -self.curr_learning_rate * biases_momenta_corrected / (np.sqrt(biases_cache_corrected) + self.epsilon)
...
for epoch in range(5001):
	...
	loss = act_loss_func.forward(layer_3.outputs, y_train)
	...
	if not epoch % 100:	# updates every 100 epochs
		print(f'epoch: {epoch}, acc: {accuracy:.3f}, loss: {loss:.3f}, eta: {optimizer.curr_learning_rate:.3f}')

	# backward pass
	act_loss_func.backward(act_loss_func.outputs, y_train)
...
for X_val, y_val in validation:
    layer_1.forward(X_val)
    act_func_1.forward(layer_1.outputs)
    layer_2.forward(act_func_1.outputs)
    act_func_2.forward(layer_2.outputs)
    layer_3.forward(act_func_2.outputs)	
    loss = act_loss_func.forward(layer_3.outputs, y_val)
    ...
    print('VALIDATION #', cnt)
    print(f'acc: {accuracy:.3f}, loss: {loss:.3f}')
    
    acc_agg += accuracy
    loss_agg += loss
    
    if (cnt < len(validation)):
        cnt += 1
        
print(f"Avg acc: {acc_agg/cnt:.3f} | Avg loss: {loss_agg/cnt:.3f}")